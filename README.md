...
- The AirPlay 2 RTSP client has a credential-store boundary and attempts
  stored-credential `/pair-verify` when credentials are available.
...
- Still missing before daily use: on-screen PIN pairing UI, wiring stored
  credentials into the foobar profile/config, full encrypted event-channel
  handling, real-device validation, and timing hardening.
